...
def bloco_rotina2(handler, err, table, idAnt):
    tk=getToken(handler, err)
...
def nome3(handler, err, table, verTk): #recebendo o token de verificação
    tk = getToken(handler, err)
...
def nome2(handler, err, table, idAnt):
    tk = getToken(handler, err)
